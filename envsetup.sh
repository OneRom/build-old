...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^orm_") ; then
       ORM_BUILD=$(echo -n $1 | sed -e 's/^orm_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $ORM_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       ORM_BUILD=
    fi
    export ORM_BUILD
...
function breakfast()
{
    target=$1
    local variant=$2
    ORM_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/orm/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the BUILD model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch orm_$target-$variant
        fi
    fi
    return $?
}
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the ORM github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var ORM_VERSION)
    ...
    if (adb shell getprop ro.orm.device | grep -q "$ORM_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $ORM_BUILD, run away!"
    fi
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $ORM_FIXUP_COMMON_OUT ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.orm.device | grep -q "$ORM_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/orm/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
